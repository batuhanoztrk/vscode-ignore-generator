...
// Type extensions for better test support
...
suite('Ignore Generator Extension Test Suite', () => {
	...
	setup(() => {
		...
		mockContext = {
			...
			asAbsolutePath: (relativePath: string) => path.join('/test/extension/path', relativePath),
	...
	suite('Command Registration', () => {
		test('should register disposable command', () => {
			...
			const registerCommandStub = sandbox.stub(vscode.commands, 'registerCommand').returns(mockDisposable);
	...
	suite('Module Structure', () => {
		...
		test('should have correct function signatures', () => {
			assert.strictEqual(myExtension.activate.length, 1, 'activate should accept one parameter');
			assert.strictEqual(myExtension.deactivate.length, 0, 'deactivate should accept no parameters');
		});
	});
	...
	suite('Basic Functionality', () => {
		test('should handle file type parsing', async () => {
			...
			const fs = require('fs');
			const fileContent = '.gitignore (Git)\n.dockerignore (Docker)\n.npmignore (Npm)';
		...
		test('should handle workspace validation', async () => {
			...
			const showQuickPickStub = sandbox.stub(vscode.window, 'showQuickPick');
			showQuickPickStub.onFirstCall().resolves({ label: '.gitignore', description: 'Git' });
