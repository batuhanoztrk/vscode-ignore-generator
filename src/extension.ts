...
async function createIgnoreFile(
  ...
  for (const template of selectedTemplates) {
    ...
    const templatePatches = patches.filter(p => p.templateName === template.label);
  ...
  try {
    ...
    const totalItems = selectedTemplates.length + patches.length;
    const patchText = patches.length > 0 ? ` and ${patches.length} patch(es)` : '';
...
async function findPatchesForTemplates(
  ...
  const templatesDir = path.join(context.extensionPath, "src", "toptal", "templates");
