...
async function createIgnoreFile(
  ...
  for (const template of selectedTemplates) {
    ...
    if (await hasPatch(context, template)) {
      appliedPatches++;
    }
  }
  ...
  try {
    ...
    if (appliedPatches > 0) {
      message += ` Applied ${appliedPatches} patch(es).`;
    }
...
async function readPatchContent(
  ...
  // Generate patch file path by replacing .gitignore with .patch
  const patchPath = template.path.replace('.gitignore', '.patch');
  const fullPatchPath = path.join(context.extensionPath, "src", patchPath);

  try {
    if (fs.existsSync(fullPatchPath)) {
      return fs.readFileSync(fullPatchPath, "utf8");
    }
    return null;
  } catch (error) {
    // If patch file cannot be read, return null
    return null;
  }
}
...
async function hasPatch(
  ...
  try {
    return fs.existsSync(fullPatchPath);
  } catch (error) {
    return false;
  }
}
